今回は、テキストマイニングのエンジニア向けに、以前ご紹介した「大規模辞書マッチングを手軽に高速化してみた」のソースコードを実験のために公開します。正式な資料は特にありませんが、下記の手順を一通り実行いただければ基本的な操作を体験できると思います。　なお、ここから進めるためにはある程度Julia言語が使えるか、Juliaを始めても抵抗がないことを想定しています。所版では非効率な部分がまだまだ沢山ありますので皆様で改良改善・共有の程よろしくお願いいたします。
...
###インストール
Julia言語の実行環境をお持ちであることご確認ください。はじめての方はJuliaの[正式ダウンロードサイト](http://julialang.org/downloads/)からバージョンv0.5.0をダウンロードし、インストールしてください。　各OSのインストール方法については[各OSのインストール詳細ページ](http://julialang.org/downloads/platform.html)をご参考下さい。
...
##Getting started
下記のコードは参考論文１にあったDAWGのサンプルを再現し、検証します。ただし、ここではDAWGの情報を疎行列及びダブル配列で保持し、それぞれのタイプでの動作を確認します。「olist」変数はマッチした単語の開始と終了位置（元本文中の位置）のタプルです。なお、サンプル辞書は変数「l」で定義しています。
...
ここまで元辞書から２種類のDAWGの生成及び参照が簡単にできると紹介しました。
次にもっと大きな辞書で作成時間と動作を確認します。

サンプル辞書はオープンソースの[neologd](https://github.com/neologd/mecab-unidic-neologd)のユーザ辞書を題材にします。規模的にも質的にも十分本パッケージの性能が発揮できるではないかと思います。辞書事態は再配布はしませんのでどうぞ下記のサイトから「mecab-unidic-user-dict-seed.xxxxxxxx.csv.xz」と言ったファイルを取得し、好きなディレクトリに解凍して下さい。ここでは「c:\tmp」とします。この実験では辞書の中にある単語で長さ３文字から１１文字までだけに絞ります。それでも約２１２万語の辞書が出来上がります。

https://github.com/neologd/mecab-unidic-neologd/tree/master/seed

対象単語数が多いかつ生成アルゴリズムの効率化が今後の課題としているため、一旦実行しますとかなり時間かかります。不安になるくらいかかりますが、焦らずにお待ち下さい。実験用の環境はCore i5-6200U @2.3GHz, メモリ8GB搭載のノートパソコンですが、作成時の統計は下記のとおりです。
...
ダブル配列ベースの生成部分は論文の生成法を無視し、著者の思い付き方式で作成したため、効率が悪くものすごく時間がかかります。配列を最小限に抑えたいためにデータの詰め込みに頑張りすぎているのかもしれません。ここはご容赦下さい。
...
最後に２種類のDAWGのパフォーマンスを比較しましょう。この作業には「BenchmarkTools」というパッケージが必要です。お持ちでない方は下記のおコマンドを実行してインストールして下さい。
...
わずかでありますが、実行時間の最小値、中央値、平均値、最大値すべての項目において疎行列ベースの方が高速であることが分かります。生成時間及び参照速度の傾向を考慮すると疎行列ベースDAWGがおすすめといった結論に至ったわけです。より技術的な詳しい情報が知りたい方は是非参考論文をご覧ください。
